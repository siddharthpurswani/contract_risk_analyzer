...
def run_rag(clauses: list[Clause]) -> list[Clause]:
    """Attach deviation_score and top matching reference to each clause."""
    ...
    for clause in clauses:
        ...
        if results:
            top_match = results[0]
            clause.deviation_score = compute_deviation_score(top_match["similarity"])
            clause.top_reference = top_match["reference_text"]
            clause.reference_similarity = top_match["similarity"]
        else:
